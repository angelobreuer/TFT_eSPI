...
void pioinit(uint32_t clock_freq) {
  ...
  // Set clock divider, frequency is set up to 2% faster than specified, or next
  // division down
  uint16_t clock_div =
      0.98 + clock_get_hz(clk_sys) / (clock_freq * 2.0); // 2 cycles per bit
  sm_config_set_clkdiv(&c, clock_div);
  // Make a single 8 words FIFO from the 4 words TX and RX FIFOs
  sm_config_set_fifo_join(&c, PIO_FIFO_JOIN_TX);
  // The OSR register shifts to the left, sm designed to send MS byte of a
  // colour first, autopull off
...
/***************************************************************************************
** Function name:           pushBlock - for generic processor and parallel
*display
...
#if defined(SPI_18BIT_DRIVER) ||                                               \
    (defined(SSD1963_DRIVER) && defined(TFT_PARALLEL_8_BIT))
  uint32_t col = ((color & 0xF800) << 8) | ((color & 0x07E0) << 5) |
                 ((color & 0x001F) << 3);
  if (len) {
    WAIT_FOR_STALL;
...
/***************************************************************************************
** Function name:           pushPixels - for generic processor and parallel
*display
...
void TFT_eSPI::pushBlock(uint16_t color, uint32_t len) {
  ...
  // If more than 32 pixels then change to 16-bit transfers with concatenated
  // pixels
  if (len > 32) {
    uint32_t rg = r << 8 | g;
    uint32_t br = b << 8 | r;
    ...
    while (len > 1) {
      ...
      spi_get_hw(SPI_X)->dr = rg;
      while (!spi_is_writable(SPI_X)) {
      };
      spi_get_hw(SPI_X)->dr = br;
      while (!spi_is_writable(SPI_X)) {
      };
      spi_get_hw(SPI_X)->dr = gb;
      len -= 2;
    }
...
#if !defined(RP2040_PIO_INTERFACE)
  dma_channel_configure(dma_tx_channel, &dma_tx_config, &spi_get_hw(SPI_X)->dr,
                        (uint16_t *)image, len, true);
#else
  dma_channel_configure(dma_tx_channel, &dma_tx_config, &tft_pio->txf[pio_sm],
                        (uint16_t *)image, len, true);
#endif
}
...
#if !defined(RP2040_PIO_INTERFACE)
  channel_config_set_dreq(&dma_tx_config,
                          spi_get_index(SPI_X) ? DREQ_SPI1_TX : DREQ_SPI0_TX);
#else
